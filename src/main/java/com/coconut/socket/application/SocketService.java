package com.coconut.socket.application;
...
//        ArrayList<String> readMembers = dto.getReadMembers();
    ...
    // 보낸 사람 채팅방 보이게 하기
    if (users.size() == 2) {socketUserChatRoom.enableChatRoom();}
    ...
    if (unReadMembers.size() == 1) {unReadMembers.get(0).getUserChatRoom(socketChatRoom.getId().toString()).enableChatRoom();}
